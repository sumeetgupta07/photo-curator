...
// v2.14 changes:
//   - getUploadByOurMediaItemId: now accepts email OR falls back to session_id
//     lookup when email is null (fixes 404 on /api/swipe when sessions table
//     has null google_email — happens if userinfo fetch failed during OAuth).
//   - Added getSessionEmail(sessionId): re-fetches google_email from sessions
//     table. Used by /api/swipe as belt-and-suspenders check.
// v2.8 changes:
...
export function getUploadByOurMediaItemId(email, ourMediaItemId) {
  if (email) {
    ...
    `).get(email, ourMediaItemId)
    if (row) return row
    // Fall through: email may not have been stamped on older rows — try session_id fallback
  }
  // v2.14 fallback: match by our_media_item_id alone (safe — IDs are globally unique)
  return db.prepare(`
    SELECT * FROM uploads
    WHERE our_media_item_id = ?
    ORDER BY id DESC LIMIT 1
  `).get(ourMediaItemId) || null
}

// v2.14: re-read email from sessions table (for null-email session recovery)
export function getSessionEmail(sessionId) {
  const row = db.prepare('SELECT google_email FROM sessions WHERE session_id = ?').get(sessionId)
