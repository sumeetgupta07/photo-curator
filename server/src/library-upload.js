...
export async function uploadBytes(accessToken, buffer, filename, mimeType) {
  ...
  if (!res.ok) { const b = await res.text(); throw new Error(`Byte upload failed: HTTP ${res.status} ${b.slice(0,200)}`) }
  return res.text()
}
export async function createMediaItem(accessToken, uploadToken, filename, albumId) {
  const body = { newMediaItems: [{ simpleMediaItem: { uploadToken, fileName: filename || undefined } }] }
  ...
  if (!res.ok && res.status !== 207) { const e = await res.text(); throw new Error(`batchCreate failed: HTTP ${res.status} ${e.slice(0,200)}`) }
  const data = await res.json()
  const result = data.newMediaItemResults?.[0]
  if (!result) throw new Error('batchCreate returned no result')
  const status = result.status
  if (status && status.code && status.code !== 0) throw new Error(`batchCreate item error: ${status.message} (code ${status.code})`)
...
export async function createAlbum(accessToken, title) {
  ...
  if (!res.ok) { const b = await res.text(); throw new Error(`createAlbum failed: HTTP ${res.status} ${b.slice(0,200)}`) }
  return res.json()
}
export async function getMediaItem(accessToken, mediaItemId) {
  ...
  if (!res.ok) { const b = await res.text(); throw new Error(`getMediaItem failed: HTTP ${res.status} ${b.slice(0,200)}`) }
  return res.json()
}
export async function batchAddMediaItems(accessToken, albumId, mediaItemIds) {
  ...
  if (!res.ok) { const b = await res.text(); throw new Error(`batchAddMediaItems failed: HTTP ${res.status} ${b.slice(0,200)}`) }
