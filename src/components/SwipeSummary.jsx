// SwipeSummary.jsx — v2.2
// PURPOSE: End-of-stack summary screen.
// v2.2 fix (issue 5): added guard so component renders safely even if
// swipeHistory is empty or store is mid-reset. Previously a black screen
// appeared after the last swipe due to an unhandled render with stale state.
...
export default function SwipeSummary({ onContinue, onReset }) {
  const swipeHistory = useAppStore(s => s.swipeHistory) || []

  const counts = swipeHistory.reduce(
    (acc, { action }) => { acc[action] = (acc[action] || 0) + 1; return acc },
    { good: 0, bad: 0, skip: 0 }
  )
  ...
  return (
    ...
    >
      ...
      <div className={styles.actions}>
        <motion.button className={styles.primaryBtn} onClick={onContinue} whileTap={{ scale: 0.96 }}>
          Back to Gallery
        </motion.button>
