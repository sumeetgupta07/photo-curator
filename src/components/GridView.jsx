...
//
// Chronological grid view of curated photos, empty state, status bar for
// picker progress, FAB to enter swipe view.
//
// v0.5: added a Cancel button to the status bar, shown only while
// pickerState === 'waiting'. Calls onCancelPicker (wired from App.jsx ->
// useMediaItems.cancelPickerSession) — see useMediaItems.js v0.5.
//
// v0.6: GridThumb now uses useAuthedImageUrl() instead of raw photoUrl().
//
// v2.0 Stage 1: token removed entirely. Auth is now a same-origin HTTP-only
// cookie (backend-managed OAuth).
//
// v2.0 Stage 2: switched from useAuthedImageUrl(item.baseUrl) to
// useAuthedMediaItemUrl(item.ourMediaItemId) — items in the store are now
// READY (re-uploaded) items, which don't carry a baseUrl at all (see
// useMediaItems.js v2.0 Stage 2 for the full flow change). This displays
// OUR re-uploaded copy, not the original Picker selection.
...
export default function GridView({ onSignOut, pickerState, onPickPhotos, onAddMorePhotos, onClearSession, onCancelPicker }) {
  ...
  const popupBlockedUrl = pickerError?.startsWith('POPUP_BLOCKED:')
    ? pickerError.replace('POPUP_BLOCKED:', '') : null
        ...
        {hasItems && (
          <>
            ...
            <div className={styles.bottomActions}>
              <button className={styles.secondaryBtn} onClick={onAddMorePhotos} disabled={isWorking}>
                + Add More Photos
              </button>
              <button className={styles.dangerBtn} onClick={onClearSession} disabled={isWorking}>
                Clear Session
              </button>
            </div>
          </>
...
function GridThumb({ item, index, onOpen }) {
  const src = useAuthedMediaItemUrl(item.ourMediaItemId, IMG_SIZES.thumb)
  return (
    <motion.button className={styles.thumb} onClick={() => onOpen(index)}
      whileTap={{ scale:0.95, opacity:0.8 }} transition={{ duration:0.1 }}>
      {src
        ? <img src={src} alt={item.filename || ''} className={styles.thumbImg} loading="lazy"
