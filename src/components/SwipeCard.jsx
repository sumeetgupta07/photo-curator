...
//
// Draggable swipe card for the current photo, with framer-motion gesture
// handling for swipe-right/left/up/down.
//
// v0.6: switched from raw photoUrl() to useAuthedImageUrl().
// v2.0 Stage 1: token prop removed — auth is now a same-origin cookie.
// v2.0 Stage 2: switched from useAuthedImageUrl(item.baseUrl) to
// useAuthedMediaItemUrl(item.ourMediaItemId) — items are now READY
// (re-uploaded) items without a baseUrl. See useMediaItems.js v2.0 Stage 2.
...
export default function SwipeCard({ item, onSwipeRight, onSwipeLeft, onSwipeUp, onSwipeDown }) {
  ...
  const src = useAuthedMediaItemUrl(item.ourMediaItemId, IMG_SIZES.full)

  async function flyOff(dir) {
    const t = {
      right: { x: FLY,   y: 0,    rotate: 20  },
      left:  { x: -FLY,  y: 0,    rotate: -20 },
      up:    { x: 0,     y: -FLY, rotate: 0   },
      down:  { x: 0,     y: FLY,  rotate: 0   },
    }
    await controls.start({ ...t[dir], opacity: 0, transition: { duration: 0.3, ease: [0.36, 0.66, 0.04, 1] } })
  }

  async function snapBack() {
    await controls.start({ x:0, y:0, rotate:0, opacity:1, transition: { type:'spring', stiffness:400, damping:30 } })
  }
  ...
  return (
    ...
    >
      {src
        ? <img src={src} alt={item.filename || ''} className={styles.photo} draggable={false}
            onError={() => { console.error('[Image] failed to load:', src.slice(0,80)) }}/>
      ...
      <div className={styles.meta}>
        {item.mediaMetadata?.creationTime && (
          <span className={styles.metaDate}>
            {new Date(item.mediaMetadata.creationTime).toLocaleDateString('en-US', { month:'short', day:'numeric', year:'numeric' })}
          </span>
        )}
